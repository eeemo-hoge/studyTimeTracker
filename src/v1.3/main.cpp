...
int main() {
    ...
    fstream outFile("../../csv/studyTimeLog.csv", ios::app);
    std::int32_t date;
    ...
    if (showData[0] == 'y' || showData[0] == 'Y') {
        ifstream inFile("../../csv/studyTimeLog.csv");
        if (inFile) {
            string line;
    ...
    //gotoのジャンプ先ラベル
    INPUT_DATA:
    outFile.open("../../csv/studyTimeLog.csv", ios::app);
    if (! outFile.is_open()) {
        ...
        cin >> studyTime;
        ofstream outFile("../../csv/studyTimeLog.csv", ios::app);
        if (!outFile) {
            cerr << "Failed to open file for writing." << endl;
            return 1;
        }else {
            std::int16_t num = 0;
            ifstream inFileForTotal("../../csv/studyTimeLog.csv");
            if (inFileForTotal) {
                string currentLine;
                while (getline(inFileForTotal, currentLine)) {
                    if (currentLine.empty()) {
                        continue;
                    }
                    stringstream ss(currentLine);
                    string cell;
                    std::int16_t colIndex = 0;
                    std::int32_t lineDate = 0;
                    std::int16_t lineTotal = 0;
                    bool valid = true;
                    while (getline(ss, cell, ',')) {
                        ...
                        } else if (colIndex == 4) { // TotalStudyTimeMinutes column
                            try {
                                lineTotal = static_cast<std::int16_t>(stoi(cell));
                            } catch (...) {
                                valid = false;
                            }
                            break;
                    ...
                    if (valid && lineDate == date) {
                        num = lineTotal; // 同じ日付の最後のTotalを保持
                    }
                }
                inFileForTotal.close();
            }
            totalStudyTime = num + studyTime;
        }
        ifstream inFile("../../csv/studyTimeLog.csv");
